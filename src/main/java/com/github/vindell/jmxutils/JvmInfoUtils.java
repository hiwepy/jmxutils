...
import com.github.vindell.jmxutils.property.JvmOSAttributes;
...
public class JvmInfoUtils {
	...
	public static Map<String, Object> info() {
		...
		// JVM属性
		for (JvmAttributes vm : JvmAttributes.values()) {
			dataMap.put(vm.getKey(), props.getProperty(vm.getKey()));
		}
		
		//获取操作系统相关信息  
		for (JvmOSAttributes vm : JvmOSAttributes.values()) {
			dataMap.put(vm.getKey(), props.getProperty(vm.getKey()));
		}
		...
		dataMap.put(JvmAttributes.JAVA_VM_PID.getKey(), runtimeMBean.getName());
		// 返回 Java 虚拟机实现名称。
		dataMap.put(JvmAttributes.JAVA_VM_NAME.getKey(), runtimeMBean.getVmName());
		// 返回 Java 虚拟机实现供应商。
		dataMap.put(JvmAttributes.JAVA_VM_VENDOR.getKey(), runtimeMBean.getVmVendor());
		// 返回 Java 虚拟机实现版本。
		...
		dataMap.put(JvmAttributes.JAVA_BOOT_CLASS_PATH.getKey(), runtimeMBean.getBootClassPath());
		...
		dataMap.put(JvmAttributes.JAVA_MANAGEMENT_SPECIFICATION_VERSION.getKey(), runtimeMBean.getManagementSpecVersion());
